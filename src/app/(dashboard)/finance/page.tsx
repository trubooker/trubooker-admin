...
const Finance = () => {
  ...
  const revenue = report?.result;
  console.log('revenue', report)

  // DriverRefund request ----------------------------------------------------------

  const driverRefundRequests = driverRefundRequest?.result?.data;
  console.log("driverRefundRequests", driverRefundRequests)
  const totalDriverRefundRequestsPages = driverRefundRequest?.result?.meta?.totalRecords;
  ...
  const passengerRefundRequests = passengerRefundRequest?.result?.data;
  const totalPassengerRefundRequestsPages =
    passengerRefundRequest?.result?.meta?.totalRecords;
  ...
  // Drivers Earnings ----------------------------------------------------------
  const totalDriverPages = driverEarnings?.result?.meta?.totalRecords;
  const driverData = driverEarnings?.result?.data;
  ...
  const totalAgentPages = agentsEarnings?.result?.meta?.last_page;
  const agentData = agentsEarnings?.result;
                      ...
                      <CardContent className="text-2xl font-semibold h-full flex flex-col my-auto justify-center gap-y-2">
                        <div className="flex gap-x-3 text-white items-center">
                          <span className="flex gap-x-2 items-center">
                            ...
                            ) : revenue?.gateway_balance_available ? (
                              <CountUp
                                prefix="₦ "
                                end={Number(revenue?.gateway_balance ?? 0)}
                              />
                            ) : (
                        ...
                        <div className="flex gap-x-3 text-white items-center">
                          <span className="flex gap-x-2 items-center">
                            {reportLoading ? (
                              ...
                              <CountUp
                                end={Number(revenue?.total_passenger_pending_withdrawals)}
                              />
                            )}
                          </span>
                        </div>
                  ...
                  <ScrollArea className="w-full">
                    <div>
                      <BarCharts
                        ...
                        graph_data={
                          Array.isArray(revenue?.revenue?.graph_data)
                            ? revenue?.revenue?.graph_data
                            : [revenue?.revenue?.graph_data]
                        }
                      />
                    </div>
