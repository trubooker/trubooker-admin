...
// NOTE: coupon_type values now match the backend CouponType enum exactly.
// Backend enum = "percentage" | "fixed" | "welcome" | "referral"
const FormSchema = z.object({
  coupon_code: z.string().min(3, "Coupon code must be at least 3 characters"),
  coupon_type: z.enum(["percentage", "fixed"]), // ← was "fixed_amount"
...
const CouponGenerator = () => {
  ...
  const onSubmit = async (data: z.infer<typeof FormSchema>) => {
    // Map form field names → backend GenerateCouponDto field names.
    // Only send fields the DTO whitelists. max_users / is_active are dropped
    // because the backend has no column/field for them yet.
    const payload: Record<string, any> = {
      code: data.coupon_code,
      type: data.coupon_type, // already "percentage" | "fixed"
      value: Number(data.coupon_value),
      usageLimit: Number(data.max_usage),
      expiresAt: data.expiry_date.toISOString(), // full ISO8601
    };

    if (data.min_order_amount) {
      payload.minOrderAmount = Number(data.min_order_amount);
    }
    if (data.description) {
      payload.description = data.description;
    }

    console.log("generate payload →", payload); // remove once confirmed working
    ...
    await generateCoupon(payload)
      ...
      .then(() => {
        ...
        toast.success(`Coupon Code: ${data.coupon_code}`, { duration: 5000 });
        form.reset();
      })
      .catch((err) => {
        console.error("generate error →", err);
        // Surface the real backend validation message if present
        const msg =
          err?.data?.errors?.[0]?.errors?.[0] ||
          err?.data?.message ||
      ...
      <CardContent className="border-none px-0 rounded-lg">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            {/* Active Status — UI only for now (backend has no field yet) */}
            <FormField
              control={form.control}
              name="is_active"
              render={({ field }) => (
