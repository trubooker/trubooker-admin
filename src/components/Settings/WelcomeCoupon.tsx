...
// coupon_type now matches backend CouponType: "percentage" | "fixed"
...
type CouponTypeT = "percentage" | "fixed";

const WelcomeCoupon = () => {
  const [isCustomCode, setIsCustomCode] = useState(false);
  const [isEnabled, setIsEnabled] = useState(false);
  const [couponType, setCouponType] = useState<CouponTypeT>("percentage");

  const {
    data: welcomeCouponData,
    isLoading: loading,
    refetch,
    ...
    form.reset({
      ...
      min_order_amount:
        v.minOrderAmount ?? v.min_order_amount
          ? String(v.minOrderAmount ?? v.min_order_amount)
          : "",
      coupon_code: v.code ?? v.coupon_code ?? "",
    });
    setIsCustomCode(!!(v.code ?? v.coupon_code) && !(v.is_auto_generated ?? false));
  }, [welcomeCouponData, form]);

  // Build a payload using EXACT backend ToggleWelcomeCouponDto field names.
  const buildPayload = (
    activate: boolean,
    data: z.infer<typeof FormSchema>
  ): Record<string, any> => {
    const payload: Record<string, any> = {
      activate, // ← required boolean the DTO checks
      type: data.coupon_type, // "percentage" | "fixed"
      value: Number(data.coupon_value),
      expiryDays: Number(data.expiry_days), // requires DTO field (see note)
      usageLimit: Number(data.usage_limit), // requires DTO field (see note)
    };
    if (isCustomCode && data.coupon_code) payload.code = data.coupon_code;
    if (data.min_order_amount) payload.minOrderAmount = Number(data.min_order_amount);
    return payload;
  };

  const surfaceError = (error: any, fallback: string) => {
    const msg =
  ...
  const onSubmit = async (data: z.infer<typeof FormSchema>) => {
    const payload = buildPayload(isEnabled, data);
    console.log("welcome coupon payload →", payload); // remove once confirmed
    try {
      await setWelcomeCoupon(payload).unwrap();
      toast.success("Welcome coupon settings saved successfully");
      refetch();
    } catch (error) {
      surfaceError(error, "Error saving settings");
    }
  };

  const handleToggleEnable = async (checked: boolean) => {
    setIsEnabled(checked);
    // Only persist immediately when disabling; enabling just reveals the form.
    if (!checked) {
      const payload = buildPayload(false, form.getValues());
      try {
        await setWelcomeCoupon(payload).unwrap();
        toast.success("Welcome coupon disabled");
        refetch();
      } catch (error) {
        setIsEnabled(true); // revert on error
        surfaceError(error, "Error updating settings");
      }
    }
  };
      ...
      <CardContent className="border-none px-0 rounded-lg">
        ...
        <div className="flex flex-row items-center justify-between rounded-lg border p-4 mb-6">
          <div className="space-y-0.5">
            <Label className="text-base">Enable Welcome Coupon</Label>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <Select
                        onValueChange={(value: CouponTypeT) => {
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>Days after registration</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>How many times can be used</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
