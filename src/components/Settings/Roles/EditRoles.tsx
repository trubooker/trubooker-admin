...
export function EditRoles({
  ...
  const handleCheckboxChange = (name: string, checked: boolean) => {
    ...
    const updatedPermissions = checked
      ? [...currentPermissions, name] // Add name if checked
      : currentPermissions.filter((permName) => permName !== name); // Remove if unchecked
  ...
  const onSubmit = async (data: z.infer<typeof FormSchema>) => {
    const formData = {
      ...
      permissions: data.permissions, // string[] of permission names
    };
            ...
            {permissionsLoading ? (
              ...
              permissions.map((perm: any) => (
                <div key={perm.name} className="flex items-center space-x-2">
                  <Checkbox
                    id={`permission-${perm.name}`}
                    checked={form.watch("permissions").includes(perm.name)}
                    onCheckedChange={(checked) =>
                      handleCheckboxChange(perm.name, !!checked)
                    }
                  />
                  <label
                    htmlFor={`permission-${perm.name}`}
                    className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
                  >
                    {formatSnakeCase(perm.name)}
                  </label>
                </div>
              ))
            )}
